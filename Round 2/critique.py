...
for i in range(T): #range function can be called with one var, 0 is implied 
    ...
    #max_len = 0 you use this to determine how many iterations to use, but it is better to just check if you have won
    for j in range(N): 
        C = input() 
        c.append(C) 
    ...
    while True: 
        ...
        # r_index, s_index, p_index = [],[],[] you store indices, why not store the whole move
        r, s, p = [],[],[]
